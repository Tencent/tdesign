<template>
  <div ref="article" name="DOC" class="doc-fonts">
    ...
    <p>
      TDesign
      秉承包容、多元、进化和连接的价值观，这将指引文字系统制定「好用」、「好记」和「美观」的字体设计原则，希望字体是有规律和韵律、实现像素对齐、可以拉开清晰明确的层次关系、
      具有和谐美观的大小对比效果。
    </p>

    <h2>字体样式</h2>
    <p>
      字体定义每个系统中所使用的字体，给到统一的字体规范，在 TDesign
      当中，需要通过字体、字阶、行高、字重、字色、几个维度去制定文字系统。
    </p>

    <h3>字体</h3>
    <p>
      font-family
      是用于某个元素的字体族名称或/及类族名称的一个优先表。如果浏览器不支持第一个字体，则会尝试优先表中的下一个。不同主流的操作系统及浏览器的默认字体不尽相同。从西文到中文，分别对个平台做一个基础的降级，这是针对系统字体规范
      font-family 的基本思路。
    </p>
    <p>
      这里需要注意的是，不声明字体时，浏览器渲染的是「默认字体」，不一定是「系统字体」例如：Windows7
      浏览器默认渲染的是中易宋体（SImsun），而非系统字体微软雅黑（Microsoft YaHei）。
    </p>
    ...
    <p>
      第一字阶，每个字号的增长的步数差距为 2px。桌面端中，Body 字号我们设置为 14px，Base 取 12px，桌面端的最小字号是
      12px，而 10px 则作为移动端的 Base 字号，将桌面端与移动端字阶统一处理，让两端文字与设计系统更一致，更标准地调控。
    </p>
    <p>
      第二字阶，为了减少字阶区间里的步数，在这个范围中的文字，多为模块超大标题，展示型文案，以数字数据展示为主。通过
      +4，+8，+12，+16 的步数增长规律，满足这个范围的字阶中，文字层级跨度大的展示诉求，也减少没有必要的小步数字号。
    </p>
    ...
    <p>
      在验证过程中发现，固定 1.5
      倍的行高比例，在当字号越大的时候，行高就会越大，在大号文字的展示上信息的连贯明显得出现割裂，尤其在多种字号及元素混排的场景中。
    </p>
    <p>
      行高是为了让上一行的文字和下一行的文字之间有呼吸的空间，基于呼吸空间一致，让不同字号之间的间距都保持相同，通过逻辑得到这样一个公式：「
      行高 = 字号 + n 」，8 作为变量正好同时满足与 1.5 倍的「 14px & 16px
      」常用字号行高保持一致，总体文字间隙稳定呼吸，行高空间较一致得出计算公式：「line-height = font size+8」
    </p>
    ...
    <p>
      面对复杂的中后台场景，在 Windows 系统下 font-weight 如果使用小于 600 默认显示为常规体，而在macOS
      系统中可以使用400、500、600区分，综合考虑复杂场景显示效果，TDesign 共提供两种字重配置效果。
    </p>
    <p>
      font-weight: 400 和 font-weight: 600。字重 400 等于 Windows 和 macOS 系统下的 Regular 显示效果，字重 600 等于
      Windows 系统下的 Bold 显示效果和 macOS 系统下的 Semibold 显示效果。
    </p>
    ...
    <p>
      根据字体样式的设计原则，制定了简易好记的透明度数值区间并且将该字色与界面系统的色彩系统结合，文字显示色彩对比满足至少1:4.5(
      AA级别）。且验证了其中的实用性，共分为亮暗两种模式，4 个色阶。
    </p>
    <p>
      注意：因为黑底白字的 Web AIM
      值的曲线和白底黑字不同，加了透明度之后辨识度是比白底黑字高的，保证一致的通用文字阅读舒适性，暗色模式平衡了 AIM
      值略微降低了透明度，确保 AIM 差值曲线是均衡的。
    </p>
